...
(define-union expr
  ...
  (lambda body)
  (app rator rand))
;Added the define union-for closure, in this case there is only one function involving closure that we need to get rid of, just make-closure
(define-union clos
  (make-closure x env))
...
;Ran into some issues with this because I didn't add that second clos parameter in the union-case
