...
;Defining the registers, which act like the registers on your actual hardware do, where they are just containers that store values
;that are reachable from anywhere within the scope of the program
;Registerizing the interpreter involves us changing the let* statements to begins, and with the global registers this allows us to remove
;arguments from our function calls, because the arguments are set to the correct values right before the function is called with begin()
    ...
    (match *a-k*
      ...
      [`(make-pitch-inner-k ,v^)
       (begin (set! *a-k* v^)
              ...
              (apply-k))]
      [`(make-let-k ,v^ ,env^ ,k^)
       (begin (set! *env-cps* (extend-env *v* env^))
              (set! *cexpr* v^)
              (set! *k* k^)
              (value-of-cps))]
    ...
    (union-case *cexpr* expr
                ...
                [(if test conseq alt)
                 (begin (set! *k* (make-if-k conseq alt *env-cps* *k*))
                        ...
                        (value-of-cps))]
                [(catch body)
                 (begin (set! *env-cps* (extend-env *k* *env-cps*))
                        (set! *k* *k*)
                        (set! *cexpr* body)
                        (value-of-cps))]
                [(pitch k-exp v-exp)
                ...
                [(var v)
                 (begin (set! *k* *k*)
                        ...
                        (apply-env))]
                [(lambda body)
                 (begin (set! *v* (make-closure body *env-cps*))
                        (set! *a-k* *k*)
                        (apply-k))]
                [(app rator rand)
