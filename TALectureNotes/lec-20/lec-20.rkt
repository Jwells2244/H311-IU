        ...
        (call/fresh 'b
          (λ (b)
            (conj
              (== 4 a)
              (== 3 b)
              ;; this can't be η-contracted
              (λ (S)
                ((== S q) S)))))))))

(run*-g!
  (λ (x)
    (conda (== 'olive x) succeed (== 'oil x))))

(run*-g!
  (λ (x)
    (disj
      (conj (== 'olive x) succeed)
      (conj succeed (== 'oil x)))))

(run*-g!
  (λ (x)
    (conda (disj (== 'olive x) (== 'apple x))
      succeed (== 'oil x))))

;;; for once, only the first line in the goal
;;; contributes to values
(run*-g!
  (λ (x)
    (conda (once (disj (== 'olive x) (== 'apple x)))
      succeed (== 'oil x))))

;;; This definition could also introduce
;;; the o variable and invoke (appendo r s o).

;;; If the same variable name is used in a
;;; call/fresh chain, then shadowing will occur,
;;; so avoid that, since it uses lexical scope.
(letrec
  ((appendo
     (λ (l1 s2 o)
       (λ (S)
