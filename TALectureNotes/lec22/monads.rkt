...
(: listen (All (W A)
            (-> (Writer W A)
              (Writer W (Pair (Listof W) A)))))
(define (listen ma)
  (match ma
    ((Writer la a) (Writer la (cons la a)))))
#;
(define (listen ma)
  (Writer (Writer-log ma)
          (cons (Writer-log ma) (Writer-a ma))))
...
(λ (bind m f)
  (cond
    ...
    (else (error 'bind "bind: ~a is not a monad" m))))

(define-syntax-rule (bind m f)
  (cond
    ((or (Just? m) (Nothing? m)) (bind-maybe m f))
    ((Writer? m) (bind-writer m f))
    ((State? m) (bind-state m f))
